...
Many newsworthy stories do not happen in a single day, but often span multiple weeks, month or even years. In these long timespans many events occur that contribute the overall development of the larger story. Often information about these incidents is spread out across a large number of different articles that add new information as the situation develops. This can make it difficult for users wanting to inform themselves about the event to get an overview of what happens. Many online publications offer a solution to the problem by creating timelines of the events. %(EXAMPLES!)
...
\label{quot:hospital}
 ...
 % corpora/syria/articles/2011-04-08/100.htm.txt: (TABLE!)
...
\begin{align}
V = U \\
E = \{ (s_1, s_2) \in V \times V \mid \exists r_1 \in \mathit{daterefs}(s_1), r_2 \in \mathit{daterefs}(s_2) \colon \\ r_2 \mathit{ contains } r_1 \land r_1 \mathit{ is exact} \} \\ % Inexact to inexact edges?
...
$f_{ ling*TR^{(date)} }$     & 0.436    & 0.279 & 0.062 & 0.059 & 0.019 & 0.069 & 0.020 \\\hline
$f_{ ling*TR^{(global)} }$   & 0.425    & 0.284 & 0.061 & 0.056 & 0.014 & 0.067 & 0.016 \\\hline
$f_{ ling*TR^{(temp)}*\mathit{dateref}*\mathit{csize}*\mathit{path} }$     & 0.445    & 0.218 & 0.050 & 0.048 & 0.015 & 0.056 & 0.016 \\\hline
$f_{ ling*TR^{(temp)}*\mathit{dateref}*\mathit{csize} }$     & 0.453    & 0.275 & 0.061 & 0.057 & 0.016 & 0.068 & 0.019 \\\hline
$f_{ \mathit{ling}*TR^{(cluster)}*\mathit{cluster size} }$   & 0.443    & 0.280 & 0.060 & 0.055 & 0.015 & 0.066 & 0.017 \\\hline
\multicolumn{ 8 }{|l|}{ \textit{ Affinity Propagation } }\\\hline
$f_{ ling*TR^{(date)} }$     & 0.485    & \textbf{ 0.383 }  & 0.085 & 0.080 & 0.025 & 0.091 & 0.027 \\\hline
$f_{ ling*TR^{(global)} }$   & 0.486    & 0.363 & 0.075 & 0.070 & 0.015 & 0.083 & 0.017 \\\hline
$f_{ ling*TR^{(temp)}*\mathit{dateref}*\mathit{csize}*\mathit{path} }$     & \textbf{ 0.538 } & 0.334 & \textbf{ 0.086 }  & 0.086 & \textbf{ 0.028 }  & 0.096 & \textbf{ 0.030 } \\\hline
$f_{ ling*TR^{(temp)}*\mathit{dateref}*\mathit{csize} }$     & 0.537    & 0.380 & 0.084 & \textbf{ 0.088 }  & 0.025 & \textbf{ 0.099 }  & 0.027 \\\hline
$f_{ \mathit{ling}*TR^{(cluster)}*\mathit{cluster size} }$   & 0.521    & 0.360 & 0.078 & 0.078 & 0.025 & 0.089 & 0.027 \\\hline
$f_{ ling*TR^{(temp)}*\mathit{dateref}*\mathit{csize}*\mathit{path} }$     & 0.525    & 0.316 & 0.078 & 0.080 & 0.027 & 0.087 & 0.028 \\\hline
...
See the following example, where we miss a verb in the subclause.
...
Extractive summarization approaches have the advantage that all sentences that they include in their documents have been found in their source documents. As long as their source documents are reliable, the sentences are thus most likely not completely incorrect, although there might be missing context.
...
While superficially there is nothing wrong with this sentence, a look at the original documents reaveals the following sentence, from which the statement was apparently generated:
